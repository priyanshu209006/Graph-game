...
class Game {
    ...
    update() {
        ...
        this.marbles.forEach(marble => {
            marble.update(this.physics, this.equations);
            ...
            this.stars.forEach(star => {
                if (!star.collected && this.physics.checkCollision(marble, star)) {
                    star.collected = true;
                    this.starsCollected++;
    ...
    launchMarbles() {
        ...
        for (let i = 0; i < Math.min(this.equations.length, 3); i++) {
            ...
            // Give marbles a small initial velocity to start moving
            marble.velocity = { x: 0.08, y: -0.02 };
